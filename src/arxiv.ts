...
const defaultGateway = createArxivGateway();
...
// Compatibility interface retained during the expand step. The package's
// public interface moves to Research Run before this adapter is contracted.
export async function searchArxivCategories(
  categories: string[],
  terms: string[],
  papersPerCategory: number,
  sinceYears: number,
  onFetch?: (fetchResult: CategoryFetch) => void,
): Promise<Paper[]> {
  const byId = new Map<string, Paper>();
  for (const category of categories) {
    const papers = await defaultGateway.search({
      category,
      terms,
      maxResults: papersPerCategory,
      sinceYears,
    });
