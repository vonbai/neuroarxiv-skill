import { createArxivGateway, mergePaper } from "./arxiv.js";
...
function looksLikeCategoryId(id) {
...
function uniqueStrings(values, name, max) {
    ...
    if (normalized.length > max)
        throw new Error(`${name} must contain at most ${max} values`);
    return normalized;
}
function normalizeCategories(categories) {
    ...
    for (const category of categories) {
        ...
        if (!looksLikeCategoryId(id))
            throw new Error(`invalid arXiv category: ${id}`);
        if (!byId.has(id)) {
            byId.set(id, { id, why: category.why.trim() || "caller-selected" });
        }
    }
    if (byId.size === 0)
        throw new Error("searchPlan.categories must not be empty");
    if (byId.size > 8)
        throw new Error("searchPlan.categories must contain at most 8 values");
    return [...byId.values()];
}
function mergeBatches(batches, maxPapers) {
    const byId = new Map();
...
function coverageFor(papers, termination) {
    if (papers.length === 0) {
        return {
...
export function createResearchEvidenceCollector(gateway) {
    return async function collect(request) {
        const problem = request.problem.trim();
        if (!problem)
            throw new Error("problem must not be empty");
        ...
        const expansionTerms = request.searchPlan.expansionTerms?.length
            ...
            : [];
        const sinceYears = boundedInteger(request.searchPlan.sinceYears, 8, "searchPlan.sinceYears", 0, 100);
        const budget = {
            maxPapers: boundedInteger(request.budget?.maxPapers, DEFAULT_BUDGET.maxPapers, "budget.maxPapers", 1, 100),
            maxFullTextPapers: boundedInteger(request.budget?.maxFullTextPapers, DEFAULT_BUDGET.maxFullTextPapers, "budget.maxFullTextPapers", 0, 20),
        };
        const attempts = [];
        const batches = [];
        async function collectPhase(phase, phaseTerms) {
            for (const category of categories) {
...
export function collectResearchEvidence(request) {
...
function validateCitation(citation, path, paperVersions, findingVersions, documentationEvidence, errors) {
...
function validateFinding(finding, index, paperVersions, errors) {
...
function validateAngle(angle, index, findingVersions, errors) {
    const path = `angles[${index}]`;
    requireText(angle.label, `${path}.label`, errors);
    if (angle.paperVersions.length === 0)
        errors.push(`${path} must contain a finding`);
...
function expectInteger(value, path, errors, min = Number.MIN_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER) {
...
function validateRetrievalFailureShape(value, path, errors) {
...
function sameRetrievalFailure(left, right) {
...
function validateResearchRunShape(input, errors) {
    ...
    if (evidence) {
        ...
        if (searchPlan) {
            const categories = expectArray(searchPlan.categories, "researchEvidence.searchPlan.categories", errors);
            categories?.forEach((item, index) => {
                const category = expectObject(item, `researchEvidence.searchPlan.categories[${index}]`, errors);
                if (category) {
            ...
            expectStringArray(searchPlan.expansionTerms, "researchEvidence.searchPlan.expansionTerms", errors);
        ...
        if (budget) {
            ...
            expectInteger(budget.maxFullTextPapers, "researchEvidence.budget.maxFullTextPapers", errors, 0, 20);
        }
        ...
        papers?.forEach((item, index) => {
            ...
            const authors = expectArray(paper.authors, `researchEvidence.papers[${index}].authors`, errors);
            if (authors?.length === 0) {
                errors.push(`researchEvidence.papers[${index}].authors must not be empty`);
            }
            authors?.forEach((author, authorIndex) => requireText(author, `researchEvidence.papers[${index}].authors[${authorIndex}]`, errors));
            const paperCategories = expectArray(paper.categories, `researchEvidence.papers[${index}].categories`, errors);
            if (paperCategories?.length === 0) {
                errors.push(`researchEvidence.papers[${index}].categories must not be empty`);
            }
            paperCategories?.forEach((category, categoryIndex) => requireText(category, `researchEvidence.papers[${index}].categories[${categoryIndex}]`, errors));
        });
        ...
        attempts?.forEach((item, index) => {
            ...
            if (!attempt)
                return;
            expectEnum(attempt.phase, ["initial", "expansion"], `researchEvidence.attempts[${index}].phase`, errors);
            ...
            expectEnum(attempt.status, ["succeeded", "failed"], `researchEvidence.attempts[${index}].status`, errors);
            expectInteger(attempt.paperCount, `researchEvidence.attempts[${index}].paperCount`, errors, 0);
            const requests = expectArray(attempt.requests, `researchEvidence.attempts[${index}].requests`, errors);
            ...
            if (attempt.status === "failed") {
                validateRetrievalFailureShape(attempt.failure, `researchEvidence.attempts[${index}].failure`, errors);
            }
        });
        const coverage = expectObject(evidence.coverage, "researchEvidence.coverage", errors);
        if (coverage) {
            expectEnum(coverage.status, ["ready", "thin", "empty", "unavailable"], "researchEvidence.coverage.status", errors);
        }
        const termination = expectObject(evidence.termination, "researchEvidence.termination", errors);
        if (termination) {
            expectEnum(termination.reason, ["initial-budget-satisfied", "search-plan-exhausted", "retrieval-failed"], "researchEvidence.termination.reason", errors);
            if (termination.reason === "retrieval-failed") {
                expectEnum(termination.phase, ["initial", "expansion"], "researchEvidence.termination.phase", errors);
    ...
    findings?.forEach((item, index) => {
        ...
        expectEnum(finding.evidenceDepth, ["abstract", "full-text"], `findings[${index}].evidenceDepth`, errors);
        expectEnum(finding.isolationStatus, ["isolated", "recovered"], `findings[${index}].isolationStatus`, errors);
    ...
    readingFailures?.forEach((item, index) => {
        ...
        expectEnum(failure.kind, ["isolation-broken"], `readingFailures[${index}].kind`, errors);
    ...
    if (run.recommendedPath !== null) {
        ...
        if (path) {
            ...
            citations?.forEach((item, index) => {
                ...
                if (!citation)
                    return;
                expectEnum(citation.source, ["paper", "documentation"], `recommendedPath.citations[${index}].source`, errors);
                expectString(citation.role, `recommendedPath.citations[${index}].role`, errors);
                if (citation.source === "paper") {
                    expectString(citation.paperVersion, `recommendedPath.citations[${index}].paperVersion`, errors);
                }
                else if (citation.source === "documentation") {
                    expectString(citation.sourceIdentity, `recommendedPath.citations[${index}].sourceIdentity`, errors);
                }
            });
        }
    }
    ...
    if (documentation) {
        expectEnum(documentation.status, ["used", "not-needed", "unavailable"], "documentationEvidence.status", errors);
...
export function validateResearchRun(input) {
    const errors = [];
    if (!validateResearchRunShape(input, errors))
    ...
    run.researchEvidence.papers.forEach((paper, index) => {
        ...
        if (Number.isNaN(Date.parse(paper.published)))
            errors.push(`${path}.published must be an ISO date`);
        if (Number.isNaN(Date.parse(paper.updated)))
            errors.push(`${path}.updated must be an ISO date`);
    ...
    const expansionAttempts = run.researchEvidence.attempts.filter((attempt) => attempt.phase === "expansion");
    const coverage = run.researchEvidence.coverage.status;
    const paperCount = run.researchEvidence.papers.length;
    const plannedCategories = run.researchEvidence.searchPlan.categories.map((category) => category.id);
    ...
    run.researchEvidence.attempts.forEach((attempt, index) => {
        ...
        if (attemptKeys.has(key))
            errors.push(`duplicate Search Attempt ${key}`);
        attemptKeys.add(key);
        if (attempt.phase === "expansion")
            sawExpansion = true;
        if (attempt.phase === "initial" && sawExpansion) {
            errors.push("initial Search Attempts must precede expansion attempts");
        }
        const expectedTerms = attempt.phase === "initial"
    ...
    const firstFailedAttempt = run.researchEvidence.attempts.findIndex((attempt) => attempt.status === "failed");
    if (firstFailedAttempt !== -1 &&
    ...
    run.researchEvidence.attempts.forEach((attempt, index) => {
        ...
        if (attempt.status === "succeeded") {
            const successfulRequests = attempt.requests.filter((request) => request.status === "succeeded");
            if (!lastRequest ||
    ...
    const successfulInitialCategories = new Set(run.researchEvidence.attempts
        .filter((attempt) => attempt.phase === "initial" && attempt.status === "succeeded")
        .map((attempt) => attempt.category));
    const successfulExpansionCategories = new Set(run.researchEvidence.attempts
    ...
    if (termination.reason === "initial-budget-satisfied" &&
    ...
    if (termination.reason === "search-plan-exhausted" &&
        run.researchEvidence.searchPlan.expansionTerms.length > 0 &&
    ...
    const readingFailureVersions = new Set();
    ...
    const fullTextCount = run.findings.filter((finding) => finding.evidenceDepth === "full-text").length;
    ...
    run.angles.forEach((angle, index) => {
        ...
        if (angleLabels.has(angle.label))
            errors.push(`duplicate Architectural Angle: ${angle.label}`);
    ...
    if (run.status === "incomplete") {
        ...
        if (run.incompleteReason === null) {
            ...
            requireText(run.incompleteReason.reentryCondition, "incompleteReason.reentryCondition", errors);
        }
    }
    ...
    if (run.incompleteReason?.kind === "research-evidence-unavailable" &&
    ...
    if (readingFailureVersions.size > 0 &&
    ...
    if (run.incompleteReason?.kind === "isolation-broken" &&
        readingFailureVersions.size === 0) {
        errors.push("isolation-broken requires at least one Reading Failure");
    }
    const usableFindingCount = run.findings.filter((finding) => paperVersions.has(finding.paperVersion)).length;
    if (run.status === "complete" &&
        (coverage !== "ready" || usableFindingCount < READY_PAPER_COUNT)) {
        errors.push(`Complete Research Run requires ready Research Evidence and at least ${READY_PAPER_COUNT} usable findings`);
    }
    if (run.status === "thin" &&
        (coverage === "unavailable" ||
            usableFindingCount === 0 ||
            usableFindingCount >= READY_PAPER_COUNT)) {
        errors.push(`Thin Coverage Research Run requires available Research Evidence and 1-${READY_PAPER_COUNT - 1} usable findings`);
    }
    if (run.recommendedPath) {
        ...
        requireText(run.recommendedPath.loadBearingRisk, "recommendedPath.loadBearingRisk", errors);
        if (run.recommendedPath.citations.length === 0) {
            errors.push("recommendedPath requires at least one Evidence Citation");
        }
        run.recommendedPath.citations.forEach((citation, index) => validateCitation(citation, `recommendedPath.citations[${index}]`, paperVersions, findingVersions, run.documentationEvidence, errors));
    }
